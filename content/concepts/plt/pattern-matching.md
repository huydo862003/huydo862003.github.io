---
published: true
author: hdnax
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: Pattern matching
journey: plt
status: learning
books:
  - chapter-5-types
dependsOn: []
blocks: []
tags: []
---

A language construct that deconstructs algebraic datatype values by variant, binding fields to variables and enabling the type checker to refine types in each branch - safer than raw predicates + acce

A language construct that **deconstructs** algebraic datatype values by variant, binding fields to variables and enabling the type checker to **refine types** in each branch - safer than raw predicates + accessors.
