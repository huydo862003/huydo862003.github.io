---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Type variable"
description: "A placeholder (e.g., $T$, $U$, $\\alpha$) representing an unknown or generic type - gets instantiated to a concrete type during type checking or remains polymorphic."
journey: plt
status: reviewing
books: ["chapter-5-types"]
dependsOn: []
blocks: []
tags:
  - "language-feature-design"
  - "syntax-grammar"
  - "type-theory"
keywords:
  - "type"
  - "variable"
  - "placeholder"
  - "alpha"
  - "representing"
  - "unknown"
  - "generic"
  - "gets"
  - "instantiated"
---

A **placeholder** (e.g., $T$, $U$, $\alpha$) representing an unknown or generic type - gets instantiated to a concrete type during type checking or remains polymorphic.
