---
published: true
author: hdnax
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: Small-step/Structural operational semantics (SOS)
description: "A style of semantics which defines evaluation via Evaluation statement/judgement of the form $t \\to t'$, specifying how a term reduces by one atomic step at a time until reaching a normal form."
journey: plt
status: learning
books:
  - chapter-3-untyped-arithmetic-expressions
dependsOn: []
blocks: []
tags: []
keywords:
  - "semantics"
  - "evaluation"
  - "form"
  - "small-step"
  - "structural"
  - "operational"
  - "sos"
  - "style"
  - "defines"
  - "via"
---

A style of semantics which defines evaluation via Evaluation statement/judgement of the form $t \to t'$, specifying how a term reduces by one atomic step at a time until reaching a normal form.
