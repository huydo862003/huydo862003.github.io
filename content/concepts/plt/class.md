---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Class"
description: "A blueprint or template that defines the structure (fields) and behavior (methods) for creating objects, serving as a factory for instantiating objects with shared characteristics."
journey: plt
status: mastered
books: ["chapter-4-objects"]
dependsOn: []
blocks: []
tags:
  - "language-feature-design"
  - "programming-paradigm"
  - "objects"
  - "class"
  - "blueprint"
  - "template"
  - "defines"
  - "structure"
  - "fields"
  - "behavior"
  - "methods"
  - "creating"
---

A blueprint or template that defines the structure (fields) and behavior (methods) for creating objects, serving as a factory for instantiating objects with shared characteristics.
