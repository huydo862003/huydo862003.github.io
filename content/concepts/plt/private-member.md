---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Private member"
description: "A class member that is only accessible from within the class itself, providing encapsulation by hiding implementation details from external code."
journey: plt
status: mastered
books: ["chapter-4-objects"]
dependsOn: []
blocks: []
tags:
  - "design-principle"
  - "language-feature-design"
  - "programming-paradigm"
  - "member"
  - "class"
  - "private"
  - "accessible"
  - "within"
  - "itself"
  - "providing"
  - "encapsulation"
  - "hiding"
---

A class member that is only accessible from within the class itself, providing encapsulation by hiding implementation details from external code.
