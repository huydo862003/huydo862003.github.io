---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Structural typing"
description: "A type system where Type compatibility is determined by structure, not name - any value with matching shape satisfies the type, regardless of declared class or origin."
journey: plt
status: mastered
books: ["chapter-5-types"]
dependsOn: []
blocks: []
tags:
  - "language-feature-design"
  - "semantics"
  - "type-theory"
keywords:
  - "type"
  - "structural"
  - "typing"
  - "system"
  - "compatibility"
  - "determined"
  - "structure"
  - "name"
  - "value"
---

A type system where Type compatibility is determined by **structure**, not **name** - any value with matching shape satisfies the type, regardless of declared class or origin.
