---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Sequence"
description: "An ordered collection of elements indexed by natural numbers - formally, a function from $\\mathbb{N}$ to some set, denoted $(a_0, a_1, a_2, ...)$ or $(a_i)_i\\in\\mathbb{N}$."
journey: plt
status: mastered
books: ["chapter-2-mathematical-preliminaries"]
dependsOn: []
blocks: []
tags:
  - "order-lattice-theory"
  - "set-theory-relation"
  - "mathbb"
  - "sequence"
  - "ordered"
  - "collection"
  - "elements"
  - "indexed"
  - "natural"
  - "numbers"
  - "formally"
  - "function"
---

An ordered collection of elements indexed by natural numbers - formally, a function from $\mathbb{N}$ to some set, denoted $(a_0, a_1, a_2, ...)$ or $(a_i)_i\in\mathbb{N}$.
