---
createdAt: "2026-03-28"
updatedAt: "2026-03-28"
title: "Type constructor"
description: "A function at the type level that builds new types from existing ones, such as List, Array, or the arrow constructor ($\\to$) that constructs function types."
journey: plt
status: mastered
books: ["chapter-5-types"]
dependsOn: []
blocks: []
tags:
  - "language-feature-design"
  - "type-theory"
  - "type"
  - "constructor"
  - "function"
  - "types"
  - "level"
  - "builds"
  - "existing"
  - "ones"
  - "list"
  - "array"
---

A function at the type level that builds new types from existing ones, such as `List`, `Array`, or the arrow constructor ($\to$) that constructs function types.
